...
final class CheckoutInventory extends BaseInventory {
	...
	CheckoutInventory( Checkout checkout) {
		super(checkout);
	}
	...
	private void loadPurchases( final BillingRequests requests,  Product product, long id) {
	...
	private abstract class ProductRequestListener<R> implements RequestListener<R> {
		...
		protected ProductRequestListener( Product product, long id) {
		...
		@Override
		public final void onError(int response,  Exception e) {
			onFinished(id);
		}
	}

	private void loadSkus( BillingRequests requests,  final Product product, long id) {
	...
	private class CheckoutListener extends Checkout.ListenerAdapter {
		...
		@Override
		public void onReady( BillingRequests requests,  String productId, boolean billingSupported) {
